...
@implementation SteppingFuckerBrain

-(id)init{
    if( (self=[super init])) {
        framecounter = 0;
        bh = [[HeroChasingBehaviour alloc]initWithGameSprite:self andForce:100 andModuloTrigger:40];
		...
		face = [[SteppingFuckerFace alloc]init];
		[self addChild:face]; 
        [face release];
    }
    return self;
...
-(NSString*)controllerType{
    return @"SteppingFuckerHealthController";
}
